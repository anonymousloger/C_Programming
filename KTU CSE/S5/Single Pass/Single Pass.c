...
void main() {
  ...
  while (strcmp(opcode, "END") != 0) {
    ...
    if (strcmp(label, "**") != 0) 535 fprintf(fp2, "%s\t%d\n", label, locctr);
    ...
    if (strcmp(opcode, "WORD") == 0) 546 locctr += 3;
    else if (strcmp(opcode, "RESW") == 0) 548 locctr += (3 * (atoi(operand)));
    else if (strcmp(opcode, "RESB") == 0) 550 locctr += (atoi(operand));
    else if (strcmp(opcode, "BYTE") == 0) 552++locctr;
    fprintf(fp3, "\t%s\t%s\t%s\n", label, opcode, operand);
    fscanf(fp1, "%s%s%s", label, opcode, operand);
  }
  ...
  while (strcmp(opcode, "END") != 0) {
    ...
    } else if ((strcmp(opcode, "RESB") == 0) || (strcmp(opcode, "RESW") == 0)) 617 fprintf(fp4, "%d\t%s\t%s\t%s\n", address, label, opcode, operand);
    else {
      while (strcmp(opcode, mnemonic1[l]) != 0) l++;
      if (strcmp(operand, "COPY") == 0) 622 fprintf(fp4, "%d\t%s\t%s\t%s\t%s0000\n", address, label, opcode, operand, code1[l]);
      else {
        rewind(fp5);
        fscanf(fp5, "%s%d", symbol, & add);
        while (strcmp(operand, symbol) != 0) fscanf(fp5, "%s%d", symbol, & add);
